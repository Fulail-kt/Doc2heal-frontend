...
const DocDashboard: FC = () => {
  ...
  useEffect(()=>{
    ...
    const fetchData = async () => {
      ...
      console.log(res.data.booking,"bookins");
          ...
          <div className='w-full flex justify-center '>
            <div className='bg-transparent bg-gray-300 w-full  flex flex-col justify-center'>
              <div className='flex flex-col items-center sm:flex sm:flex-row sm:items-start md:flex-row md:items-start gap-x-4 mt-1 m-3 '>
                <div className=' bg-slate-700 w-[90%] sm:w-1/2 md:w-1/2 rounded-md h-96 shadow-2xl '> {loading?(<Spinner/>):(<ChartExample data={data.modifiedData} />)} </div>
                ...
                {/* <div className='bg-slate-500 m-1 h-28 rounded-md w-1/2'>{data?.totalEarnings}</div> */}
                </div>
              </div>
               <div className='flex flex-col-reverse items-center sm:flex sm:flex-row sm:items-start md:flex-row md:items-start gap-x-4 mt-0 m-3'>
                ...
                {/* <div className='bg-slate-500 m-1 h-28 rounded-md w-1/2'></div> */}
                </div>
                <div className=' bg-slate-700 relative right-0 mb-1 mt-1 sm:mb-0 md:mb-0  w-[90%] sm:mt-[-36%] md:mt-[-26%] lg:mt-[-14%] md:w-[49%] sm:w-[49%]  rounded-md h-96 shadow-2xl   '> {loading?(<Spinner/>):(<PieChart data={booking}/>)} </div>
              </div> 
            </div>
          </div>
        </div>
